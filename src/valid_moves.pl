...
find_all_moves(Board, Player, NonCapturingMoves, CapturingMoves, ListOfMoves) :-
    findall(Move, (
        ...
        generate_moves(Row, Col, NonCapturingMoves, Board, Player, Move),
        generate_moves(Row, Col, CapturingMoves, Board, Player, Move)
    ), ListOfMoves).


% Generate non-capturing or capturing moves for a given piece
generate_moves(Row, Col, Directions, Board, Player, move(Row, Col, Direction, NewRow, NewCol)) :-
    member(Direction, Directions),
    new_position(Row, Col, Direction, NewRow, NewCol), % Compute the new position
    is_valid_move(Board, Player, NewRow, NewCol, Direction). % Validate the move
